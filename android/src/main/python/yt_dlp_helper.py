...
def extract_format_info(format_data):
    ...
    return {
        ...
        "resolution": format_data.get("resolution", "unknown") if format_data.get("vcodec", "none") != "none" else "audio only",
        "bitrate": int(format_data.get("tbr", 0) or 0),
        "size": int(format_data.get("filesize", 0) or format_data.get("filesize_approx", 0) or 0),
...
def download_format(url, format_id, output_path, overwrite, progress_callback):
    ...
    def hook(d):
        if d["status"] == "downloading":
            ...
            total = d.get("total_bytes", d.get("total_bytes_estimated", 0) or 0)
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            logger.info(f"Downloading {url} format {format_id} to {output_path}")
            ydl.download([url])
